...
function generateCatchWeight(species) {
    const weights = PORTUGUESE_FISHING_DATA.FISH_WEIGHTS[species];
    if (!weights)
        return 500; // fallback для неизвестных видов
...
function generateFishingDepth(species) {
    const depths = PORTUGUESE_FISHING_DATA.FISHING_DEPTHS[species];
    if (!depths)
        return 50; // fallback для неизвестных видов
...
async function createCatchRecords() {
    ...
    for (let date = new Date(startDate); date <= endDate; date.setDate(date.getDate() + 1)) {
        ...
        for (let i = 0; i < numCatches; i++) {
            ...
            const lunarPhase = getRandomItem(lunarPhases); // Temporary type fix
            const weather = getRandomItem(WEATHER_CONDITIONS);
            // Рассчитываем модификаторы
            const seasonalMod = PORTUGUESE_FISHING_DATA.SEASONAL_ACTIVITY[species]?.[season] || 0.5;
            const lunarMod = LUNAR_INFLUENCE[lunarPhase?.type] || 1.0;
            ...
            if (successMod < 0.3 && Math.random() > 0.4)
                continue;
